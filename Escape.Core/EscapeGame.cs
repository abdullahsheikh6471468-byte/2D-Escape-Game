...
using Escape.Core.Effects;
using Escape.Core.Localization;
using Escape.Core.Settings;
using Escape.ScreenManagers;
using Escape.Screens;
...
namespace Escape.Core
{
    ...
    public class EscapeGame : Game
    {
        ...
        private SettingsManager<EscapeSettings> settingsManager;

        // Manages leaderboard data for tracking high scores and achievements.
        private SettingsManager<EscapeLeaderboard> leaderboardManager;

        // Manages saved escape-game progress (unlocked/completed levels, scores).
        ...
        public EscapeGame()
        {
            ...
            settingsManager = new SettingsManager<EscapeSettings>(storage);
            Services.AddService(typeof(SettingsManager<EscapeSettings>), settingsManager);

            leaderboardManager = new SettingsManager<EscapeLeaderboard>(storage);
            Services.AddService(typeof(SettingsManager<EscapeLeaderboard>), leaderboardManager);

            progressManager = new SettingsManager<GameProgress>(CreateProgressStorage());
            Services.AddService(typeof(SettingsManager<GameProgress>), progressManager);
        ...
        /// <summary>
        /// Creates a settings storage instance dedicated to escape-game save
        /// progress, using its own file so it never overwrites the shared
        /// settings/leaderboard file.
        /// </summary>
        private ISettingsStorage CreateProgressStorage()
        {
            ISettingsStorage progressStorage = IsMobile ? new MobileSettingsStorage() : new DesktopSettingsStorage();
            progressStorage.SettingsFileName = "progress.json";
